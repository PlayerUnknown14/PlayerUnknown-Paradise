...
/obj/machinery/atmospherics/reactor_chamber
	...
	max_integrity = 400
	armor = list(melee = 80, bullet = 30, laser = 30, energy = 10, bomb = 40, fire = INFINITY, acid = INFINITY) // Fairly robust
...
/obj/machinery/atmospherics/reactor_chamber/examine(mob/user)
	...
	if(length(deep_info))
		. += boxed_message(deep_info.Join("<br>"))
	...
	switch(chamber_state)
		if(CHAMBER_DOWN, CHAMBER_OVERLOAD_IDLE)
			...
			if(do_after(user, delay, target = src))
				if(!IsReachableBy(user)) // For mecha users
					return
				if(density_check(user))
					raise()
				return
...
/obj/machinery/atmospherics/reactor_chamber/attack_hand_secondary(mob/user, list/modifiers)
	. = ..()

	. = SECONDARY_ATTACK_CANCEL_ATTACK_CHAIN

	if(!IsReachableBy(user) || lockout)
		return
...
/obj/machinery/atmospherics/reactor_chamber/update_icon_state()
	switch(chamber_state)
		if(CHAMBER_DOWN, CHAMBER_OVERLOAD_ACTIVE)
			icon_state = "chamber_down"
		if(CHAMBER_OPEN)
			icon_state = "chamber_open"
		if(CHAMBER_UP)
			icon_state = "chamber_up"
		if(CHAMBER_OVERLOAD_IDLE)
...
/obj/machinery/atmospherics/reactor_chamber/proc/open(playsound = TRUE)
	...
	flick_overlay_view(cover_icon, 0.5 SECONDS)
	lockout = TRUE
...
/datum/milla_safe/chamber_process/on_run(obj/machinery/atmospherics/reactor_chamber/chamber)
	var/turf/location = get_turf(chamber)
...
/obj/machinery/atmospherics/reactor_chamber/proc/eject_rod()
	...
	smoke.set_up(amount = 5, location = loc)
	smoke.start()
...
/obj/machinery/atmospherics/reactor_chamber/proc/burn_handler(mob/user)
	...
	if(ishuman(user))
		...
		if(human_user.gloves)
			...
			if(gloves.max_heat_protection_temperature)
				burn_damage *= 0.5
		var/obj/item/organ/external/affecting = human_user.get_organ(user.hand ? BODY_ZONE_PRECISE_L_HAND : BODY_ZONE_PRECISE_R_HAND)
		if(affecting.external_receive_damage(0, burn_damage)) // Burn damage to them fingers
			human_user.UpdateDamageIcon()
			human_user.updatehealth()
		return
	...
	if(issilicon(living_user)) // More resistant by default
		burn_damage *= 0.5
	living_user.adjustFireLoss(burn_damage)
...
/obj/effect/immovablerod/nuclear_rod/Bump(atom/clong)
	if(istype(clong, /obj/machinery/atmospherics/fission_reactor))
		return

	if(istype(clong, /obj/structure/filler))
		var/obj/structure/filler/filler = clong
		if(istype(filler.parent, /obj/machinery/atmospherics/fission_reactor))
			return

	if(istype(clong, /obj/machinery/atmospherics/reactor_chamber))
		return

	return ..()

/obj/effect/immovablerod/nuclear_rod/Moved(atom/old_loc, movement_dir, forced, list/old_locs, momentum_change)
	. = ..()
	if(loc == end)
		qdel(src)
