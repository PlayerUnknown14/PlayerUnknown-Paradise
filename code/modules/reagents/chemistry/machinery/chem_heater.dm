/obj/machinery/chem_heater
	...
	desc = "Простая машина, представляющая собой камеру для проведения химических реакций. \
...
/obj/machinery/chem_heater/get_ru_names()
	return alist(
		NOMINATIVE = "реакционная камера",
		GENITIVE = "реакционной камеры",
		DATIVE = "реакционной камере",
		ACCUSATIVE = "реакционную камеру",
		INSTRUMENTAL = "реакционной камерой",
		PREPOSITIONAL = "реакционной камере",
	)
...
/obj/machinery/chem_heater/add_context(atom/source, list/context, obj/item/held_item, mob/user)
	...
	if(!QDELETED(beaker))
		if(istype(held_item, /obj/item/reagent_containers/dropper) || istype(held_item, /obj/item/reagent_containers/syringe))
			context[SCREENTIP_CONTEXT_LMB] = "Впрыснуть содержимое"
...
/obj/machinery/chem_heater/RefreshParts()
	...
	for(var/obj/item/stock_parts/micro_laser/micro_laser in component_parts)
		heater_coefficient *= micro_laser.rating
...
/obj/machinery/chem_heater/proc/move_buffer(datum/reagent/buffer_type, volume)
	...
	if(volume < 0)
		if(!beaker.reagents.has_reagent(buffer_type))
			...
			atom_say("Unable to find [name] in beaker to draw from! Please insert a beaker containing [name].")
			return FALSE
