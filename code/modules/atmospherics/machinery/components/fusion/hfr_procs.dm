 ...
 */
/obj/machinery/atmospherics/unary/hypertorus/core/proc/check_part_connectivity()
	. = TRUE
	...
	for(var/obj/machinery/atmospherics/unary/hypertorus/object in orange(1, src))
		...
		if(istype(object,/obj/machinery/atmospherics/unary/hypertorus/waste_output))
			if(linked_output && linked_output != object)
				. = FALSE
			linked_output = object
			machine_parts |= object

		if(istype(object,/obj/machinery/atmospherics/unary/hypertorus/moderator_input))
 ...
 */
/obj/machinery/atmospherics/unary/hypertorus/core/proc/activate(mob/living/user)
	if(active)
 ...
 */
/obj/machinery/atmospherics/unary/hypertorus/core/proc/unregister_signals(only_signals = FALSE)
	SIGNAL_HANDLER
	if(linked_interface)
 ...
 */
/obj/machinery/atmospherics/unary/hypertorus/core/proc/deactivate()
	if(!active)
		return
	active = FALSE
	update_appearance(UPDATE_ICON)
 ...
 */
/obj/machinery/atmospherics/unary/hypertorus/core/proc/update_pipenets()
	parent?.update = TRUE
	linked_input.parent?.update = TRUE
	linked_output.parent?.update = TRUE
	linked_moderator.parent?.update = TRUE

/obj/machinery/atmospherics/unary/hypertorus/core/proc/update_temperature_status(seconds_per_tick)
	...
	coolant_temperature = air_contents.temperature()
	output_temperature_archived = output_temperature
	output_temperature = linked_output.air_contents.temperature()
 ...
 */
/obj/machinery/atmospherics/unary/hypertorus/core/proc/play_ambience(seconds_per_tick)
 ...
 */
/obj/machinery/atmospherics/unary/hypertorus/core/proc/check_fuel()
	if(!selected_fuel)
		return FALSE
	if(!internal_fusion.total_moles())
		return FALSE
	var/list/gas_list = internal_fusion.get_interesting()
	for(var/gas_type in selected_fuel.requirements)
		if(!gas_list[gas_type] || gas_list[gas_type] < FUSION_MOLE_THRESHOLD)
			return FALSE
	return TRUE
...
/obj/machinery/atmospherics/unary/hypertorus/core/proc/check_gas_requirements()
	var/datum/gas_mixture/contents = linked_input.air_contents
	var/list/gas_list = contents.get_interesting()
	for(var/gas_type in selected_fuel.requirements)
		if(!gas_list[gas_type])
			return FALSE
	return TRUE

///Removes the gases from the internal gasmix when the recipe is changed
/obj/machinery/atmospherics/unary/hypertorus/core/proc/dump_gases()
	var/datum/gas_mixture/remove = internal_fusion.remove(internal_fusion.total_moles())
	linked_output.air_contents.merge(remove)

/**
 * Called by alarm() in this file
 * Check the integrity level and returns the status of the machine
 */
/obj/machinery/atmospherics/unary/hypertorus/core/proc/get_status()
 ...
 */
/obj/machinery/atmospherics/unary/hypertorus/core/proc/alarm()
	switch(get_status())
		if(HYPERTORUS_MELTING)
			playsound(src, 'sound/announcer/bloblarm.ogg', 100, FALSE, 40, 30, falloff_distance = 10)
 ...
 */
/obj/machinery/atmospherics/unary/hypertorus/core/proc/get_integrity_percent()
 ...
 */
/obj/machinery/atmospherics/unary/hypertorus/core/proc/get_area_cell_percent()
	...
	if(!area)
		return 0
	var/obj/machinery/power/apc/apc = area.apc?[1]
	if(!apc)
		return 0
 ...
 */
/obj/machinery/atmospherics/unary/hypertorus/core/proc/check_alert()
	...
	if((REALTIMEOFDAY - lastwarning) / 10 >= WARNING_TIME_DELAY)
		...
		if(critical_threshold_proximity > emergency_point)
			radio_announce(
				"[emergency_alert] Integrity: [get_integrity_percent()]%",
			...
			radio_announce(
				"[warning_alert] Integrity: [get_integrity_percent()]%",
				DECLENT_RU_CAP(src, NOMINATIVE),
				engineering_channel
			)
			...
			radio_announce(
				"[safe_alert] Integrity: [get_integrity_percent()]%",
				DECLENT_RU_CAP(src, NOMINATIVE),
				engineering_channel
			)
			lastwarning = REALTIMEOFDAY
...
/obj/machinery/atmospherics/unary/hypertorus/core/emp_act(severity)
	. = ..()
	//if(. & EMP_PROTECT_SELF)
		//return
 ...
 */
/obj/machinery/atmospherics/unary/hypertorus/core/proc/send_radio_explanation()

	if(warning_damage_flags & HYPERTORUS_FLAG_EMPED)
		var/list/characters = list()
		characters += GLOB.alphabet
		characters += GLOB.alphabet_uppercase
	...
	if(warning_damage_flags & HYPERTORUS_FLAG_HIGH_POWER_DAMAGE)
		radio_announce(
			"Warning! Shield destabilizing due to excessive power!",
			DECLENT_RU_CAP(src, NOMINATIVE),
			engineering_channel
		)
	if(warning_damage_flags & HYPERTORUS_FLAG_IRON_CONTENT_DAMAGE)
		radio_announce(
			"Warning! Iron shards are damaging the internal core shielding!",
			DECLENT_RU_CAP(src, NOMINATIVE),
			engineering_channel
		)
	if(warning_damage_flags & HYPERTORUS_FLAG_HIGH_FUEL_MIX_MOLE)
		radio_announce(
			"Warning! Fuel mix moles reaching critical levels!",
			DECLENT_RU_CAP(src, NOMINATIVE),
			engineering_channel
		)
	if(warning_damage_flags & HYPERTORUS_FLAG_IRON_CONTENT_INCREASE)
		radio_announce(
			"Warning! Iron amount inside the core is increasing!",
			DECLENT_RU_CAP(src, NOMINATIVE),
			engineering_channel
		)
 ...
 */
/obj/machinery/atmospherics/unary/hypertorus/core/proc/countdown()
	set waitfor = FALSE
	...
	if(critical)
		GLOB.major_announcement.announce("WARNING - The explosion will likely cover a big part of the station and the coming EMP will wipe out most of the electronics. \
	...
	for(var/i in HYPERTORUS_COUNTDOWN_TIME to 0 step -10)
		if(critical_threshold_proximity < melting_point) // Cutting it a bit close there engineers
			radio_announce(
				"[safe_alert] Failsafe has been disengaged.",
				DECLENT_RU_CAP(src, NOMINATIVE),
				common_channel
			)
 ...
 */
/obj/machinery/atmospherics/unary/hypertorus/core/proc/meltdown()
	...
	if(internal_fusion.total_moles() > 0)
		...
		location.blind_release_air(internal_fusion)
	...
	if(moderator_internal.total_moles() > 0)
		...
		for(var/i in 1 to gas_pockets)
			...
			var/turf/local = pick(around_turfs)
			local.blind_release_air(remove)
		location.blind_release_air(moderator_internal)
 ...
 */
/obj/machinery/atmospherics/unary/hypertorus/core/proc/check_cracked_parts()
	for(var/obj/machinery/atmospherics/unary/hypertorus/part in machine_parts)
		if(part.cracked)
			return part
 ...
 */
/obj/machinery/atmospherics/unary/hypertorus/core/proc/create_crack() as /obj/machinery/atmospherics/unary/hypertorus
	var/obj/machinery/atmospherics/unary/hypertorus/part = pick(machine_parts)
	part.cracked = TRUE
 ...
 */
/obj/machinery/atmospherics/unary/hypertorus/core/proc/spill_gases(obj/origin, datum/gas_mixture/target_mix, ratio)
 ...
 */
/obj/machinery/atmospherics/unary/hypertorus/core/proc/process_moderator_overflow(seconds_per_tick)
	var/obj/machinery/atmospherics/unary/hypertorus/cracked_part = check_cracked_parts()
