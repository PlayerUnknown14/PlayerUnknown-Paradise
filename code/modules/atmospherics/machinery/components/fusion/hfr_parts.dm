...
/obj/machinery/atmospherics/unary/hypertorus/Initialize(mapload)
	...
	LAZYADD(component_parts, new circuit(null))

/obj/machinery/atmospherics/unary/hypertorus/examine(mob/user)
	. = ..()
...
/obj/machinery/atmospherics/unary/hypertorus/welder_act(mob/living/user, obj/item/tool)
	if(!cracked)
		return FALSE
...
/obj/machinery/atmospherics/unary/hypertorus/crowbar_act(mob/living/user, obj/item/tool)
	return crowbar_deconstruction_act(user, tool)

/obj/machinery/atmospherics/unary/hypertorus/update_icon_state()
	if(panel_open)
...
/obj/machinery/atmospherics/unary/hypertorus/update_overlays()
	. = ..()
	if(cracked)
...
/*
/obj/machinery/atmospherics/unary/hypertorus/update_layer()
	return
*/
/obj/machinery/atmospherics/unary/hypertorus/fuel_input
...
/obj/machinery/atmospherics/unary/hypertorus/waste_output
...
/obj/machinery/atmospherics/unary/hypertorus/moderator_input
...
/obj/machinery/hypertorus/interface
	...
	var/obj/machinery/atmospherics/unary/hypertorus/core/connected_core
...
/obj/machinery/hypertorus/interface/attack_hand(mob/user)
	if(..())
		return TRUE
...
/obj/machinery/hypertorus/interface/multitool_act(mob/living/user, obj/item/I)
	...
	var/obj/machinery/atmospherics/unary/hypertorus/core/centre = locate() in T
	...
	if(!activated)
		new /obj/item/paper/guides/hypertorus(loc)
		activated = TRUE
...
/obj/machinery/hypertorus/interface/ui_data()
	...
	var/list/internal_fusion_gas_list = connected_core.internal_fusion.get_interesting()
	for(var/datum/gas/gas_path as anything in subtypesof(/datum/gas))
		var/gas_id = gas_path.id
		fusion_gasdata.Add(list(list(
			"id" = gas_id,
			"amount" = round(internal_fusion_gas_list[gas_id] || 0, 0.01),
		)))
	//Moderator gases
	var/list/moderator_gasdata = list()
	var/list/moderator_internal_gas_list = connected_core.moderator_internal.get_interesting()
	for(var/datum/gas/gas_path as anything in subtypesof(/datum/gas))
		var/gas_id = gas_path.id
		moderator_gasdata.Add(list(list(
			"id" = gas_id,
			"amount" = round(moderator_internal_gas_list[gas_id] || 0, 0.01),
		)))
...
/obj/machinery/hypertorus/interface/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		...
		if("fuel")
			...
			connected_core.parent?.update = TRUE //prevent the machine from stopping because of the recipe change and the pipenet not updating
			connected_core.linked_input.parent?.update = TRUE
			connected_core.linked_output.parent?.update = TRUE
			connected_core.linked_moderator.parent?.update = TRUE
		...
		if("cooling_volume")
			...
			if(cooling_volume != null)
				connected_core.air_contents.volume = clamp(cooling_volume, 50, 2000)
				. = TRUE
...
/obj/item/paper/guides/hypertorus
...
/obj/item/hfr_box
	...
	var/box_type = BOX_TYPE_IMPOSSIBLE
...
/obj/item/hfr_box/body
	...
	box_type = BOX_TYPE_BODY
	icon_state = "box_body"

/obj/item/hfr_box/body/fuel_input
	name = "HFR box fuel input"
	icon_state = "box_fuel"
	part_path = /obj/machinery/atmospherics/unary/hypertorus/fuel_input

/obj/item/hfr_box/body/moderator_input
	name = "HFR box moderator input"
	icon_state = "box_moderator"
	part_path = /obj/machinery/atmospherics/unary/hypertorus/moderator_input

/obj/item/hfr_box/body/waste_output
	name = "HFR box waste output"
	icon_state = "box_waste"
	part_path = /obj/machinery/atmospherics/unary/hypertorus/waste_output
...
/obj/item/hfr_box/core
	...
	icon_state = "box_core"
	box_type = BOX_TYPE_CORE
	part_path = /obj/machinery/atmospherics/unary/hypertorus/core
...
/obj/item/hfr_box/core/multitool_act(mob/living/user, obj/item/I)
	...
	for(var/obj/item/hfr_box/box in orange(1,src))
		...
		if(box.box_type == BOX_TYPE_CORNER)
...
/obj/item/hfr_box/core/proc/build_reactor(list/parts)
	for(var/obj/item/hfr_box/box in parts)
		...
		if(box.box_type == BOX_TYPE_BODY)
			...
			if(box.part_path != /obj/machinery/hypertorus/interface)
				var/obj/machinery/atmospherics/unary/hypertorus/part = new box.part_path(location, TRUE, box.dir)
				part.dir = box.dir
			else
	...
	new/obj/machinery/atmospherics/unary/hypertorus/core(loc, TRUE)
	qdel(src)
