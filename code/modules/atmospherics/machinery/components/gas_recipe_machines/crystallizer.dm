...
/obj/machinery/atmospherics/binary/crystallizer
	...
	icon = 'icons/obj/pipes_and_stuff/atmospherics/crystallizer.dmi'
...
/obj/machinery/atmospherics/binary/crystallizer/Initialize(mapload)
	...
	internal = new
	register_context()
	LAZYADD(component_parts, new /obj/item/circuitboard/machine/crystallizer(null))

/obj/machinery/atmospherics/binary/crystallizer/on_deconstruction(disassembled)
	var/turf/local_turf = get_turf(loc)
	if(internal.total_moles())
		local_turf.blind_release_air(internal)
	return ..()

/obj/machinery/atmospherics/binary/crystallizer/add_context(atom/source, list/context, obj/item/held_item, mob/user)
	. = ..()
...
/obj/machinery/atmospherics/binary/crystallizer/crowbar_act(mob/living/user, obj/item/tool)
...
/obj/machinery/atmospherics/binary/crystallizer/proc/inject_gases()
	var/datum/gas_mixture/contents = air2
	var/list/recipe_requirements = selected_recipe.requirements
	var/list/internal_gasses = internal.get_interesting()
	for(var/gas_id, amount in contents.get_interesting())
		if(!(gas_id in recipe_requirements))
			continue
		if(internal_gasses[gas_id] >= selected_recipe.requirements[gas_id] * 2)
			continue
		internal.merge(contents.remove_specific(gas_id, amount * gas_input))

///Checks if the gases required are all inside
/obj/machinery/atmospherics/binary/crystallizer/proc/internal_check()
	var/gas_check = 0
	var/list/recipe_requirements = selected_recipe.requirements
	for(var/gas_id, amount in internal.get_interesting())
		if(!(gas_id in recipe_requirements))
			continue
		if(amount >= selected_recipe.requirements[gas_id])
			gas_check++
...
/obj/machinery/atmospherics/binary/crystallizer/proc/heat_calculations()
	var/internal_temperature = internal.temperature()
	var/progress_amount_to_quality = MIN_PROGRESS_AMOUNT * 4.5 / (round(log(10, total_recipe_moles * 0.1), 0.01))
	if((internal_temperature >= (selected_recipe.min_temp * MIN_DEVIATION_RATE) && internal_temperature <= selected_recipe.min_temp) || \
		(internal_temperature >= selected_recipe.max_temp && internal_temperature <= (selected_recipe.max_temp * MAX_DEVIATION_RATE)))
	...
	if(internal_temperature >= (median_temperature * MIN_DEVIATION_RATE) && internal_temperature <= (median_temperature * MAX_DEVIATION_RATE))
		quality_loss = max(quality_loss - progress_amount_to_quality, -85)

	internal.set_temperature(max(internal_temperature + (selected_recipe.energy_release / internal.heat_capacity()), TCMB))
	parent1?.update = TRUE
	parent2?.update = TRUE
...
/obj/machinery/atmospherics/binary/crystallizer/proc/heat_conduction()
	var/datum/gas_mixture/cooling_port = air1
	if(cooling_port.total_moles() > MINIMUM_MOLE_COUNT)
		if(internal.total_moles() > 0)
			...
			cooling_port.set_temperature(max(cooling_port.temperature() - cooling_heat_amount / cooling_heat_capacity, TCMB))
...
/obj/machinery/atmospherics/binary/crystallizer/ui_interact(mob/user, datum/tgui/ui)
	ui = SStgui.try_update_ui(user, src, ui)
	if(!ui)
		ui = new(user, src, "Crystallizer", name)
		ui.open()

/obj/machinery/atmospherics/binary/crystallizer/ui_static_data()
	var/data = list()
...
/obj/machinery/atmospherics/binary/crystallizer/ui_data()
	var/data = list()
	data["on"] = on
	...
	var/list/internal_gas_list = internal.get_interesting()
	for(var/datum/gas/gas_path as anything in subtypesof(/datum/gas))
		var/gas_id = gas_path.id
		internal_gas_data.Add(list(list(
			"name"= gas_meta[gas_id][META_GAS_NAME],
			"id" = gas_id,
			"amount" = internal_gas_list[gas_id] || 0,
		)))
...
/obj/machinery/atmospherics/binary/crystallizer/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	. = ..()
	if(.)
		return
	switch(action)
		...
		if("recipe")
			...
			if(recipe && recipe.id != "")
				...
				parent1?.update = TRUE
				parent2?.update = TRUE //prevent the machine from stopping because of the recipe change and the pipenet not updating
				moles_calculations()
	...
	update_icon()
/*
/obj/machinery/atmospherics/binary/crystallizer/update_layer()
	return
*/
