...
/obj/machinery/teleport/hub/Initialize(mapload)
	...
	component_parts += new /obj/item/circuitboard/machine/teleporter_hub(null)
...
/obj/machinery/teleport/perma/RefreshParts()
	for(var/obj/item/circuitboard/machine/teleporter_perma/C in component_parts)
		target = C.target
	var/A = 40
	for(var/obj/item/stock_parts/matter_bin/M in component_parts)
		A -= M.rating * 10
...
/obj/machinery/teleport/station/Initialize(mapload)
	. = ..()
	component_parts = list()
	component_parts += new /obj/item/circuitboard/machine/teleporter_station(null)
...
/obj/machinery/teleport/station/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/circuitboard/machine/teleporter_perma))
		add_fingerprint(user)
		if(!panel_open)
			to_chat(user, span_warning("Open th panel first!"))
			return ATTACK_CHAIN_PROCEED
		var/obj/item/circuitboard/machine/teleporter_perma/circuit = I
