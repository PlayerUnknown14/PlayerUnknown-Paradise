...
/obj/machinery/tcomms/core/proc/handle_message(datum/tcomms_message/tcm)
	...
	if(tcm.reject || !active || (machine_stat & NOPOWER) || ion)
		return FALSE
...
/obj/machinery/tcomms/core/proc/refresh_zlevels()
	...
	for(var/obj/machinery/tcomms/relay/R in linked_relays)
		...
		if(R.active && !(R.machine_stat & NOPOWER))
...
/obj/machinery/tcomms/core/proc/check_power_on()
	...
	for(var/obj/machinery/tcomms/core/C in GLOB.tcomms_machines)
		...
		if(C.active)
			if(C.machine_stat & NOPOWER)	// If another core has no power but is supposed to be on, we shut it down so we can continue.
